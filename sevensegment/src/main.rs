...
/// Static configuration whether sleep should instantly turn all LEDs on/off or
/// slowly fade them on/off
...
/// Set to have just below 500mA current draw.
const MAX_BRIGHTNESS: u8 = 255;
/// XXX calibrate
// Provide an alias for our BSP so we can switch targets quickly.
// Uncomment the BSP you included in Cargo.toml, the rest of the code does not need to change.
use bsp::entry;
use fl16_inputmodules::ssd_animations::*;
use fl16_inputmodules::ssd_hal as bsp;
...
const DEFAULT_SERIAL: &str = "CSASSDXX0100000000";
...
fn main() -> ! {
    ...
    let mut usb_dev = UsbDeviceBuilder::new(&usb_bus, UsbVidPid(0x32ac, 0x0023))
        .manufacturer("C. Scott Ananian")
        .product("Seven Segment Display Input Module")
    ...
    let mut state = SevenSegmentState {
        grid: percentage(0),
    ...
    if show_startup_animation(&state) {
        state.upcoming_frames = Some(match get_random_byte(&rosc) % 4 {
            ...
            1 => Animation::Breathing(BreathingIterator::default()),
            2 => Animation::Spin(SpinIterator::default()),
            3 => Animation::Wave(WaveIterator::default()),
            _ => unreachable!(),
        });
    } else {
    ...
    let mut ssdisp = SevenSegment::configure(i2c);
    ssdisp
        .setup(&mut delay)
    ...
    ssdisp
        .device
        .sw_enablement(is31fl3729::SwSetting::Sw1Sw9)
        .unwrap();

    for digit in 0..9 {
        ssdisp
            .set_scaling(digit, MAX_BRIGHTNESS)
            .expect("failed to set scaling");
    }

    ssdisp.device.set_pwm_freq(state.pwm_freq.into()).unwrap();

    fill_grid_pixels(&state, &mut ssdisp);
    ...
    loop {
        ...
        if dip1.is_low().unwrap() {
            // allow DIP switch to force debug mode high, but not force it low
            // (ie, let the --debug-mode flag from the control app toggle this)
                ...
                Ok(count) => {
                    ...
                    match (parse_command(count, &buf), &state.sleeping) {
                        ...
                        (Some(c @ Command::BootloaderReset), _) => {
                            handle_command(&c, &mut state, &mut ssdisp, &mut ws2812, random);
                        }
                        (Some(command), _) => {
...
fn dyn_sleep_mode(state: &SevenSegmentState) -> SleepMode {
    if state.debug_mode {
        SleepMode::Debug
    } else {
        SLEEP_MODE
...
fn debug_mode(state: &SevenSegmentState) -> bool {
    dyn_sleep_mode(state) == SleepMode::Debug
}

fn show_startup_animation(state: &SevenSegmentState) -> bool {
        ...
        (SleepState::Awake, Some(sleep_reason)) => {
            ...
            if dyn_sleep_mode(state) == SleepMode::Fading {
                ...
                loop {
                    ...
                    set_brightness(state, brightness, ssdisp);
                    if brightness == 0 {
                        break;
                    }
                }
